...
const getChild = (children, displayName) => {
    return React.Children.map(children, (child) => child.type.name === displayName ? child : null);
}
...
const Period = ({ children, className = '', ...props}) => {
    return (
        <p className={'w-60 mt-1 shrink-0 text-sm uppercase tracking-widest text-gray-400 sm:text-right ' + className} {...props}>
            {children}
        </p>
    )
}
...
const Company = ({ children, className = '', ...props}) => {
    return (
        <p className={'text-xl text-teal-400 font-bold ' + className} {...props}>{children}</p>
    )
}
...
const Content = ({ children, className = '', ...props}) => {
    return (
        <p className={'mt-4 ' + className} {...props}>{children}</p>
    )
}
